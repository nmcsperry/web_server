...
bool32 Char8Match(char8 Char, char8_class CharClass, u32 MatchFlags)
{
	...
	{
		for (int CharIndex = 0; CharIndex < CharClass.IncludeChars.Count; CharIndex++)
		{
...
i32 Str8Find(str8 String, str8 Match, match_flags MatchFlags)
{
	...
	// loop though string
	for (i32 StringIndex = 0; StringIndex < String.Count; StringIndex++) {
...
{
	Substr.Count += Extend;
	int SubstrOffset = Substr.Data - String.Data;
	if (Substr.Count > String.Count - SubstrOffset)
	{
		Substr.Count = String.Count - SubstrOffset;
	}
	return Substr;
}
...
void Str8WriteFloat(memory_buffer * Buffer, f64 Float)
{
	...
	u64 DecimalPartNumber = DecimalPart * DecPowers[FloatPrintPrecision];
...
void Str8WriteAndUnescapeStr8(memory_buffer * Buffer, str8 EscapedString)
{
	...
	for (int CharIndex = 0; CharIndex < EscapedString.Count; CharIndex++)
	{
...
void Str8WriteAndEscapeStr8(memory_buffer * Buffer, str8 UnescapedString, bool32 AddQuotes)
{
	...
	for (int i = 0; i < UnescapedString.Count; i++)
	{
...
void Str8WriteBase64Decode(memory_buffer * Buffer, str8 String, char8 Padding)
{
	...
	for (int CharIndex = 0; CharIndex < String.Count; CharIndex += 4)
	{
		byte Bytes[3];
		byte DigitValues[4];
