...
str8 FileInput(file_handle File, memory_arena * Arena)
{
	...
	u32 MaxCount = MaxCountU64 > U32Max ? U32Max : (u32) MaxCountU64;
	...
	if (FileSize <= MaxCount - 1)
	{
		...
		void * Memory = ArenaPushBytes(Arena, (u32) FileSize + 1, 32);

		u32 BytesRead = 0;
		ReadFile(WinHandle, Memory, FileSize, &BytesRead, 0);

		((u8 *) Memory)[BytesRead] = '\0';

		DWORD Error = GetLastError();
	
		str8 Result = (str8) { .Data = Memory, .Count = (u32) FileSize };
...
{
	u32 FileSize = (u32) FileGetSize(File);
