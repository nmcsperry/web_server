...
u64 PeekBits(bit_str * BitString, u8 GetCount)
{
	...
	u32 ByteIndex = BitString->Endianness == Endianness_LittleEndian ? 0 : BitString->String.Count - 1;
...
i64 PopBitsSigned(bit_str * BitString, u8 BitCount)
{
	...
	u64 Result = PeekBits(BitString, BitCount);
	if (((Result >> BitCount) - 1) & 1) // todo: its possible this is supposed to be ((Result >> (BitCount - 1)) & 1)
	{
		Result |= ~((1 << BitCount) - 1);
	}
